...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        setSlug(savedSlug)
        try { localStorage.setItem("family:slug", savedSlug) } catch {}

        const localPid = (typeof localStorage !== "undefined" ? localStorage.getItem(`project:${savedSlug}`) : null) || nextProjects[0]?.id || ""
  ...
  function createProject(){
    ...
    try { localStorage.setItem(`project:${slug}`, id) } catch {}
  ...
  function removeProject(id){
    ...
    if(selectedProjectId===id){
      ...
      try { localStorage.setItem(`project:${slug}`, first) } catch {}
    }
  }
  ...
  const groupedByCategory = useMemo(()=>{
    const m=new Map(); filteredDateAsc.forEach(e=>{ const arr=m.get(e.category)||[]; arr.push(e); m.set(e.category,arr) })
  ...
  const settlements = useMemo(()=>{
    const set=new Set(filtered.map(e=>e.who)); const arr=[...set]; const share=arr.length?total/arr.length:0
    const paid=Object.fromEntries(arr.map(p=>[p,0])); filtered.forEach(e=>paid[e.who]+=(e.amount||0))
    ...
    while(i<receivers.length&&j<payers.length){
      ...
      receivers[i].diff-=take; payers[j].diff+=take
      if(receivers[i].diff<0.009)i++; if(payers[j].diff>-0.009)j++
    }
    return {delta,ops}
  },[filtered,total])
      ...
      <div className={`mt-4 p-3 ${card}`}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 dark:bg-slate-900" value={selectedProjectId} onChange={e=>{
            setSelectedProjectId(e.target.value); try { localStorage.setItem(`project:${slug}`, e.target.value) } catch {}
          }}>
